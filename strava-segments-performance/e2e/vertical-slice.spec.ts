...
//   1. Authenticated user lands on /dashboard (fixture already navigates).
...
test('picker narrows fitness-trend chart end-to-end', async ({ page }) => {
  // The fixture navigated to /dashboard before the test body runs; the initial analysis
  // request is already in flight, so declare the wait AFTER goto but before assertions
  // that depend on the response. Any response matching the analysis endpoint counts —
  // the seed guarantees the initial call arrives before any picker interaction.
