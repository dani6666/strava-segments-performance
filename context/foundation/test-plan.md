...
| 4 | OAuth handshake round-trip + config safety | Prove Risk #2 the full challenge→callback→cookie→redirect round-trip completes against a stubbed Strava (success + failure branches, redirect_uri from forwarded headers), cookie/auth config resolves per environment, and the 401 branch holds | #2 | integration / config + browser e2e (stub provider) + deploy smoke | complete | context/archive/2026-09-02-testing-oauth-roundtrip/ |
| 5 | Vertical-slice happy-path e2e | Prove Risk #7 an authenticated date-range → analysis → chart happy path renders end-to-end (browser e2e, reusing the Phase 4 Playwright runner) | #7 | browser e2e (stub-provider login, seeded data) | change opened | context/changes/testing-vertical-slice-happy-path/ |
...
- 2026-09-03: §3 Phase 4 reconciled to `complete`. The change folder was archived with automated items 1.1–5.3 done but manual items 5.4–5.6 unchecked. 5.4/5.5 were confirmed via the real master deploy's "OAuth redirect smoke (post-deploy)" CI step passing (GitHub Actions run `33760072676`, commit `2e216ac`). 5.6 (forced misconfig makes the gate fail) had never actually been exercised — the earlier CI failure on this step was an unrelated sed-portability bug, not a deliberate negative test — so it was run manually: a throwaway local backend + Postgres, hit directly without `X-Forwarded-Proto`, produced an `http://` redirect_uri and `oauth-redirect-smoke.sh` correctly failed; a forwarded-header positive control on the same instance passed. See plan.md 5.4–5.6 for detail.
